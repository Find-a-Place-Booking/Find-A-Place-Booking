...
-- Hotfix: store SQL NULL, not JSON null, when no promotion is applied.
--
-- Current failure:
--   new row for relation "reservations" violates check constraint
--   "reservations_promotion_snapshot_check"
--
-- reservations.promotion_snapshot permits:
--   - SQL NULL when there is no promotion
--   - a JSON object when a promotion exists
--
-- quote_unit_stay returns JSON null for the promotion key when no code is used.
-- `quote -> 'promotion'` therefore produced JSONB 'null', which is not a JSON
-- object and violates the reservation constraint.
--
-- Safe to apply after the sandbox guest booking migration/hotfixes.
...
comment on function public.create_sandbox_guest_reservation_hold(
  ...
  'Service-role-only sandbox guest hold creator. Stores SQL NULL for promotion_snapshot when no promotion is applied.';
