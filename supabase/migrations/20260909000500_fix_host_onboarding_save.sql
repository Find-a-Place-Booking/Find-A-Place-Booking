...
-- Milestone 6 hotfix: remove ambiguous local names from save_host_onboarding
--
-- Migration 004 is already applied in development. Do not edit/re-run it just
-- to fix this issue; this migration safely replaces only the affected RPC.
...
comment on function public.save_host_onboarding(uuid, integer, jsonb, text[], text[], text[], boolean) is
  'Persists host onboarding for an owned/managed organization. Hotfix removes PL/pgSQL column/local ambiguity for contact fields. Host claims can never self-grant PARTNER_5.';
