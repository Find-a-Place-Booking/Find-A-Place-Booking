...
-- Direct-charge payment model.
--
-- Hosts are the Stripe merchant of record for booking charges. Stripe charges
-- processing fees to the connected host account. Find A Place collects only
-- its application fee (platform commission plus any tax amount that the
-- platform is already configured to retain/remit).

begin;

-- Find A Place no longer schedules or creates host bank payouts. Existing
-- historical payout rows remain for audit/history, but new direct-charge
-- payments/refunds must not create or mutate reservation_payouts rows.
...
begin
  ...
  if reservation_row.tax_status = 'CALCULATED'
     and platform_tax <> reservation_row.tax_total_cents then
  ...
  application_fee := least(
    ...
    greatest(reservation_row.platform_commission_cents + platform_tax, 0)
  );
...
create or replace function public.direct_charge_payment_model_version()
returns text
language sql
stable
security definer
set search_path = ''
as $$
  select 'direct-charge-payment-model-051-v1'::text;
$$;

revoke all on function public.direct_charge_payment_model_version() from public;
grant execute on function public.direct_charge_payment_model_version()
  to anon, authenticated;

comment on function public.direct_charge_payment_model_version() is
  'Marker for Stripe connected-account direct charges, host-paid processing fees, and Stripe-managed host payouts.';
