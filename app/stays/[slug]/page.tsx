...
export default async function PropertyPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return <><Header /><main className="property-page">
    ...
    <div className="shell property-content"><article className="property-copy">
      ...
      <h2>About this stay</h2><p className="lead-copy">{property.description}</p>
      <hr/><h3>What this place offers</h3><div className="amenity-grid">{property.amenities.length ? property.amenities.map((amenity)=><span key={amenity}>✓ {amenity}</span>) : <span>Amenities are being finalized.</span>}</div>
      {property.customAmenities ? <p className="listing-custom-copy">{property.customAmenities}</p> : null}
      <hr/><h3>Know before you book</h3><div className="amenity-grid">{property.policies.map((policy)=><span key={policy}>✓ {policy}</span>)}</div>
