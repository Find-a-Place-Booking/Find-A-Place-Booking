...
export default async function CancelledBookingPage({
  ...
  const refundPending =
    status === "pending" || status === "refund_pending";
  ...
  const copy = requested
    ...
    : refundPending
      ? "The host approved the guest refund and Find A Place transmitted it against the host's connected payment charge. Do not submit a duplicate request while the processor finishes the refund."
      : declined
        ...
        : noRefund
          ...
          : "The cancellation was recorded for this reservation. Any guest refund status is tracked separately with the booking and payment processor.";
  ...
  return (
    <main className="confirm-page">
      ...
      <section className="confirm-card guest-state-card">
        ...
        <p className="muted">
          Ordinary cancellation and refund decisions are handled by the host
          under the property terms accepted at booking, subject to applicable
          law. Find A Place records the request, communication and payment
          status. Find A Place&apos;s host-paid platform commission remains
          earned and non-refundable when a paid reservation is later cancelled,
          refunded, shortened or changed.
        </p>

        <Link className="button" href="/trip">
          Open My Trip
        </Link>
        <Link className="confirm-secondary" href="/stays">
          Find another stay
        </Link>
      </section>
    </main>
