...
async function requireFinanceAdmin() {
  ...
  if (!hasAnyAdminRole(context, ["SUPER_ADMIN", "FINANCE_ADMIN"])) taxRedirect("error", "Finance-admin access is required for tax changes.");
  return context;
}
function normalizeCode(value: string) {
  return value.toUpperCase().replace(/[^A-Z0-9_]+/g, "_").replace(/^_+|_+$/g, "").slice(0, 80);
}
...
export async function savePropertyTaxAssistance(formData: FormData) {
  ...
  const localSalesRateBps = percentToBps(field(formData, "localSalesRatePercent", 20) || "0");
  const localLodgingRateBps = percentToBps(field(formData, "localLodgingRatePercent", 20) || "0");
  const localLodgingLabel = field(formData, "localLodgingLabel", 160) || null;

  if (!propertyId || localSalesRateBps == null || localLodgingRateBps == null) {
    taxRedirect("error", "Complete the property tax fields with valid rates.");
  }

  const admin = createAdminClient();
  const { error } = await admin.rpc("service_save_property_tax_assistance", {
    ...
    local_lodging_label_value: localLodgingLabel,
  });
  if (error) taxRedirect("error", error.message);

  revalidatePath("/admin/taxes");
  revalidatePath("/host/payments");
  taxRedirect("saved", "Tax configuration saved as admin assistance. The host must review and certify it.");
}
...
export async function createTaxAuthority(formData: FormData) {
  ...
  const allowedTypes = new Set(["STATE_SALES","STATE_TOURISM","COUNTY_SALES","CITY_SALES","LOCAL_LODGING","OTHER"]);
  if (!code || !name || !remittanceAgency || !allowedTypes.has(jurisdictionType)) taxRedirect("error", "Complete the tax authority fields.");

  const admin = createAdminClient();
  const { error } = await admin.from("tax_authorities").insert({
    code,name,jurisdiction_type:jurisdictionType,country_code:countryCode,region_code:regionCode,
    locality_name:localityName,remittance_agency:remittanceAgency,source_url:sourceUrl,is_active:true,
  });
  if (error) taxRedirect("error", error.message);
  revalidatePath("/admin/taxes");
...
export async function createTaxRule(formData: FormData) {
  ...
  const maximumStayNights = maximumStayText ? Number.parseInt(maximumStayText, 10) : null;
  const sourceUrl = field(formData, "sourceUrl", 1000) || null;

  if (!authorityId || !code || !label || rateBps == null || !effectiveFrom ||
      !["LODGING_ONLY","ACCOMMODATION_TOTAL","PRE_TAX_TOTAL"].includes(baseScope) ||
      (maximumStayNights != null && (!Number.isInteger(maximumStayNights) || maximumStayNights < 1))) {
    taxRedirect("error", "Complete the tax rule fields with a valid rate.");
  }

  const admin = createAdminClient();
  const { error } = await admin.from("tax_rules").insert({
    authority_id:authorityId,code,label,rate_bps:rateBps,base_scope:baseScope,
    automatic_region:false,assignment_required:true,maximum_stay_nights:maximumStayNights,
    effective_from:effectiveFrom,effective_to:effectiveTo,source_url:sourceUrl,is_active:true,
  });
  if (error) taxRedirect("error", error.message);
  revalidatePath("/admin/taxes");
  taxRedirect("saved", "Tax rule created for reference.");
}
