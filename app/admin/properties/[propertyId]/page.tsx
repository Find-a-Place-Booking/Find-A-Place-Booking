...
import { reviewPropertyListing, setPropertyPublication } from "@/app/admin/actions";
...
export default async function AdminPropertyDetailPage({
  ...
  const { data: property } = await supabase.from("properties").select("id,organization_id,name,description,property_type,status,public_area,street_address,city,region_code,postal_code,country_code,exact_address_public,notification_email,operations_email,calendar_preference,custom_amenities,custom_policies,submitted_at,approved_at,published_at,reviewed_at,review_note,created_at,updated_at").eq("id", propertyId).maybeSingle();
  ...
  const [ratesResult, feesResult, imagesResult, amenitiesResult, policiesResult, rateRulesResult, stayRulesResult, addOnsResult, promotionsResult] = await Promise.all([
    ...
    supabase.from("promotion_codes").select("id,unit_id,code,label,discount_type,percent_bps,amount_cents,eligible_check_in_start,eligible_check_in_end,minimum_nights,max_redemptions,redemption_count,is_active").eq("organization_id", property.organization_id).order("created_at", { ascending: false }),
  ]);
  ...
  return <AdminShell active="properties" eyebrow="Property review" title={property.name} context={context}>
    ...
    {canReview && ["APPROVED", "PAUSED"].includes(property.status) ? <section className="panel admin-publication-actions"><div><p className="eyebrow dark">Publication</p><h2>{property.status === "PAUSED" ? "Return this approved listing to the marketplace" : "Publish this approved listing"}</h2><p>Publication makes the safe guest-facing listing visible. Availability, checkout and payment are not enabled yet.</p></div><form action={setPropertyPublication}><input type="hidden" name="property_id" value={property.id} /><input type="hidden" name="publication_action" value="publish" /><label><span>Optional publication note</span><input name="publication_note" placeholder="Internal note" /></label><button className="button" type="submit">Publish to marketplace →</button></form></section> : null}
