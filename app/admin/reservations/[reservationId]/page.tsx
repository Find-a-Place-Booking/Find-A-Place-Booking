...
import {
  addReservationSupportNote,
  issueReservationRefund,
} from "../actions";
...
export default async function AdminReservationDetailPage({
  ...
  const [
    ...
    supabase
      .from("payments")
      .select(
        "id,provider,status,provider_payment_id,provider_charge_id,amount_cents,application_fee_cents,platform_tax_retained_cents,processor_fee_actual_cents,processor_fee_host_share_cents,processor_fee_platform_share_cents,host_proceeds_cents,currency,failure_code,failure_message,created_at,updated_at,payment_environment",
      )
    ...
    supabase
      .from("refunds")
      .select(
        "id,status,provider_refund_id,amount_cents,platform_fee_refund_cents,application_fee_refund_status,application_fee_refund_error,currency,reason,created_at,payment_environment",
      )
      .eq("reservation_id", reservationId)
      .eq("payment_environment", environment)
      .order("created_at", { ascending: false }),
  ...
  const canRefund = hasAnyAdminRole(context, [
    "SUPER_ADMIN",
    "FINANCE_ADMIN",
  ]);
  ...
  return (
    <AdminShell
      active="reservations"
      eyebrow={`${environment} reservation support`}
    ...
    >
      <div className="property-editor-heading">
        ...
        <span className="status-pill">
          {environment} · {readable(reservation.status)}
        </span>
      </div>
      ...
      <div className="dash-grid metrics">
        ...
        <div>
          ...
          <strong>
            {money(reservation.tax_total_cents, reservation.currency)}
          </strong>
      ...
      <div className="dash-two">
        ...
        <section className="panel">
          ...
          <div className="setting-row">
            ...
            <strong>{organization?.contact_phone || "Not set"}</strong>
          </div>
          <div className="setting-row">
            <span>Connected processor account</span>
            <strong>
              {reservation.provider_account_ref || "Not routed"}
            </strong>
          </div>
          <div className="setting-row">
            <span>Payment environment</span>
            <strong>{reservation.payment_environment}</strong>
          </div>
          <Link
      ...
      {cancellationRequest ? (
        <section className="panel">
          ...
          <div className="setting-row">
            ...
            <strong>{readable(cancellationRequest.requested_by)}</strong>
          </div>
          <div className="setting-row">
            <span>Requested</span>
            <strong>
          ...
          <p className="muted">
            Find A Place records and transmits the request. The host
            makes the ordinary booking cancellation decision under the
            accepted property policy, subject to applicable law.
          </p>
        </section>
      ) : null}
      ...
      <section className="panel">
        ...
        {payments.length ? (
            ...
            {payments.map((payment) => (
              ...
              >
                <span>
                  ...
                  <small>
                    {payment.provider_payment_id || "No payment ID"}
                  </small>
                  <small>
                    {payment.provider_charge_id || "No charge ID"}
                  </small>
                ...
                <span>
                  ...
                  <small>
                    Stripe processing charged to host account:{" "}
                    {money(
          ...
          <div className="panel-empty">
            <strong>No {environment} payment record.</strong>
          </div>
        )}

        <p className="muted">
          This support view is isolated to the active {environment} Stripe
          environment. Test payments never appear in the live Vercel
          view, and live payments never appear in the local test view.
        </p>
              ...
              >
                ...
                <span>
                  ...
                  {Number(refund.platform_fee_refund_cents) > 0 ? (
                    <small>
                      Application-fee return{" "}
                      {money(
                        refund.platform_fee_refund_cents,
                        refund.currency,
                      )}{" "}
                      ·{" "}
                      {readable(
                        refund.application_fee_refund_status,
                      )}
                      {refund.application_fee_refund_status ===
                      "FAILED"
      ...
      {paidPayment && refundableCents > 0 && canRefund ? (
        <section className="panel">
          ...
          <p className="muted">
            This refund can only operate on the active {environment}
            environment. Ordinary guest cancellation requests remain
            handled by the host.
          </p>

          <form
            className="settings-form"
            action={issueReservationRefund}
          >
            <input
              type="hidden"
              name="reservation_id"
            ...
            <label>
              ...
              <select name="refund_type" defaultValue="partial">
                ...
                <option value="full">
                  ...
                  {money(refundableCents, reservation.currency)})
                </option>
              </select>
            </label>
            <label>
      ...
      <div className="dash-two">
        ...
        <section className="panel">
          ...
          <h2>
            {review ? `${review.rating}/5` : "No review yet"}
          </h2>
          {review ? (
            <>
              <p>
                {review.body ||
                  "Rating submitted without written review."}
              </p>
            ...
            <p className="muted">
              Only a guest tied to this completed reservation can
      ...
      <section className="panel">
        ...
        <form
          className="settings-form"
          action={addReservationSupportNote}
        >
          <input
            type="hidden"
            name="reservation_id"
