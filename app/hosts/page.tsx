...
export default function HostsPage() {
  return (
    <>
      ...
      <main>
        <section className="hosts-value shell"><div className="section-heading"><div><p className="eyebrow dark">What Find A Place handles</p><h2>Built to help travelers find and book the right stay.</h2></div></div><div className="host-value-grid"><article><h3>Show up when travelers are searching</h3><p>Your listing can appear when guests search by destination, dates and group size.</p></article><article><h3>Give guests the details they need</h3><p>Photos, amenities, house rules, rates and booking details stay together on one complete property page.</p></article><article><h3>Manage hosting in one place</h3><p>Reservations, calendars, rates, fees, guest messages, reports and listing details stay within your host dashboard.</p></article></div></section>
        <section className="host-preview-section"><div className="shell host-preview-grid"><div className="host-preview-copy"><p className="eyebrow dark">Built for everyday hosting</p><h2>Keep bookings, availability and guest details within easy reach.</h2><p>Your host dashboard brings reservations, calendar availability, rates, messages, payouts and listing details together, so the day-to-day is easier to manage.</p><Link className="under-link" href="/host/sign-in">Open the host portal →</Link></div><div className="host-ui-preview empty-host-preview"><div className="preview-top"><span>Host dashboard</span><strong>Ready when you are</strong></div><div className="preview-calendar">{Array.from({length:28},(_,i)=><span key={i}>{i+1}</span>)}</div><div className="preview-booking empty-preview-booking"><div><small>Next step</small><strong>Add your first property</strong><span>Bookings and arrivals will show here as they come in.</span></div></div></div></div></section>
