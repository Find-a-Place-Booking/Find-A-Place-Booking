...
export async function GET() {
  ...
  try {
    ...
    const [
      ...
      supabase
        .from("properties")
        .select("id,live_checkout_enabled,time_zone")
        .limit(1),
      ...
      supabase
        ...
        .select("id,allow_with_public_special,archived_at")
        .limit(1),
      supabase.from("calendar_connections").select("id,sync_status").limit(1),
      supabase
        .from("availability_blocks")
        .select("id,block_type,state,reservation_id")
        .limit(1),
      ...
      supabase
        .from("reservations")
        .select(
          "id,status,payment_status,payment_environment,tax_provider,tax_provider_calculation_id,tax_provider_transaction_id,guest_email_verified_at,identity_verification_status",
        )
        .limit(1),
      supabase
        .from("payment_accounts")
        .select("id,provider,status,environment")
        .limit(1),
      supabase
        .from("payments")
        .select("id,provider,status,payment_environment")
        .limit(1),
      supabase
        .from("refunds")
        .select("id,status,payment_environment")
        .limit(1),
      supabase.from("financial_ledger_entries").select("id,entry_type").limit(1),
      supabase
        .from("processor_events")
        .select("id,provider,payment_environment,processing_status")
        .limit(1),
      supabase
        .from("notification_deliveries")
        .select("id,status,notification_type,subject,text_body,html_body")
        .limit(1),
      supabase
        .from("guest_email_verifications")
        .select("reservation_id,verified_at,attempt_count")
        .limit(1),
      supabase
        .from("reservation_policy_acceptances")
        .select("reservation_id,accepted_at,platform_terms_version")
        .limit(1),
      supabase
        .from("reservation_payouts")
        .select(
          "id,status,payment_environment,cancellation_cutoff_at,payout_eligible_at",
        )
        .limit(1),
      supabase
        .from("property_tax_profiles")
        .select("property_id,verification_status")
        .limit(1),
    ...
    const error =
      ...
      hardeningVersionError ??
      preLiveVersionError ??
      liveSafetyVersionError ??
      finalRegressionVersionError;
      ...
      return NextResponse.json(
        {
          ...
          service: "supabase",
          configured: true,
          message: "Supabase is reachable, but the current hardening migrations are not complete.",
        },
        { status: 503 },
      );
    }
