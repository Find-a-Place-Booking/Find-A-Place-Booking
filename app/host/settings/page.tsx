...
export default async function SettingsPage({ searchParams }: { searchParams: Promise<{ avatarSaved?: string; avatarRemoved?: string; avatarError?: string }> }) {
  ...
  return <DashboardShell active="Settings" title="Settings">
    ...
    <div className="dash-two">
      ...
      <section className="panel"><p className="eyebrow dark">Account identity</p><h2>Signed-in host</h2><div className="setting-row"><span>Email</span><strong>{profile.email || "Not set"}</strong></div><div className="setting-row"><span>Phone</span><strong>{profile.phone || "Not set"}</strong></div><p className="muted">Identity and contact editing can be expanded with team and account management.</p></section>
    </div>
