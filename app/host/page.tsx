...
export default async function HostDashboard() {
  ...
  return <DashboardShell active="Overview" title="Host dashboard">
    <div className="host-alert"><div><span>{propertyCount ? "✓" : ready ? "✓" : organization ? Math.max(1, Math.ceil(progress / 25)) : "1"}</span><p><strong>{propertyCount ? `${propertyCount} property ${propertyCount === 1 ? "listing is" : "listings are"} connected.` : ready ? "Host setup is complete." : organization ? `Continue setting up ${organization.name}.` : "Finish your host profile."}</strong>{propertyCount ? " Published listings can sync calendars, receive bookings and feed reservation details into this dashboard." : ready ? " Your account is ready for you to add your first property." : organization ? ` You’re ${progress}% through the guided setup.` : " Add your hosting or business details first, then build your property listing."}</p></div><Link href={propertyCount || ready ? "/host/properties" : "/host/onboarding"}>{propertyCount ? "Open properties →" : ready ? "Add property →" : organization ? "Continue setup →" : "Start setup →"}</Link></div>

    <div className="dash-grid metrics"><div><span>Upcoming booking value</span><strong>{new Intl.NumberFormat("en-US", { style: "currency", currency: "USD", maximumFractionDigits: 0 }).format(bookedRevenueCents / 100)}</strong><small>Confirmed future stays</small></div><div><span>Upcoming stays</span><strong>{upcomingStayCount}</strong><small>{upcomingStayCount ? "Confirmed reservations" : "No upcoming reservations"}</small></div><div><span>Properties</span><strong>{propertyCount}</strong><small>{publishedCount ? `${publishedCount} live` : pendingReviewCount ? `${pendingReviewCount} in review` : propertyCount ? "Draft / not published" : ready ? "Ready to add first property" : "Complete host setup first"}</small></div><div><span>Commission tier</span><strong>{organization?.commission_tier === "PARTNER_5" ? "5%" : "7%"}</strong><small>{organization?.partner_status === "VERIFIED" ? "Verified partner" : organization?.partner_status === "PARTNER_PENDING" ? "Partner review pending" : "Standard host"}</small></div></div>

    <div className="dash-two"><section className="panel"><div className="panel-head"><div><p className="eyebrow dark">Reservations</p><h2>Upcoming stays</h2></div><Link href="/host/reservations">View reservations</Link></div><div className="panel-empty"><strong>{upcomingStayCount ? `${upcomingStayCount} confirmed upcoming ${upcomingStayCount === 1 ? "stay" : "stays"}.` : "No upcoming reservations."}</strong><span>{upcomingStayCount ? "Open reservations for guest, payment and trip details." : "New confirmed Find A Place bookings will appear here automatically."}</span></div></section><section className="panel occupancy-panel"><div className="panel-head"><div><p className="eyebrow dark">Availability</p><h2>Calendar</h2></div><Link href="/host/calendar">Open calendar</Link></div><div className="panel-empty"><strong>{calendarConnectionCount ? `${calendarConnectionCount} external ${calendarConnectionCount === 1 ? "calendar is" : "calendars are"} connected.` : "No external calendar connected."}</strong><span>{calendarConnectionCount ? "Connected feeds sync automatically and are checked again before a booking is confirmed." : "Connect Airbnb, Vrbo or another iCal feed from the property calendar."}</span></div></section></div>

    <div className="dash-two"><section className="panel"><div className="panel-head"><div><p className="eyebrow dark">Account checklist</p><h2>Get ready for bookings</h2></div><span className={`status-pill ${ready ? "" : "status-muted"}`}>{ready ? "Host setup saved" : organization ? "In progress" : "Not started"}</span></div><div className="setup-list"><div><b>{organization ? "✓" : "1"}</b><span><strong>Host profile</strong><small>{organization ? organization.name : "Add the person or business managing the stays"}</small></span></div><div><b>{ready ? "✓" : "2"}</b><span><strong>Property setup details</strong><small>{ready ? "Saved and ready for property creation" : "Add amenities, rates, policies and partner information"}</small></span></div><div><b>{propertyCount ? "✓" : "3"}</b><span><strong>Property listing</strong><small>{propertyCount ? `${propertyCount} property listing${propertyCount === 1 ? "" : "s"} saved` : "Create your listing from the setup details"}</small></span></div><div><b>{calendarConnectionCount ? "✓" : "4"}</b><span><strong>Calendar &amp; payout account</strong><small>Connect your calendar and complete payout setup</small></span></div></div></section><section className="panel performance host-promotion-panel"><p className="eyebrow dark">Promotion &amp; reach</p><h2>Want help getting in front of more travelers?</h2><div className="panel-empty"><strong>Ask about a custom Find A Place advertising plan.</strong><span>Optional social media and travel-community promotion can be built around your property, location, season or campaign goals. This is separate from the booking platform commission.</span><div className="host-promotion-actions"><Link className="button button-small" href="/contact#advertising">Ask about advertising</Link><a href="https://www.findaplacear.com" target="_blank" rel="noreferrer">See the Find A Place network ↗</a></div></div></section></div>
  </DashboardShell>;
}
