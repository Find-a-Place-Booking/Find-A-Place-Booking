  ...
  return <DashboardShell active="Properties" title={property.form.name || "Property"} eyebrow="Property editor">
    ...
    <section className="panel property-review-submit">
      <div>
        ...
        {canSubmit ? property.submissionIssues.length ? <><p>Finish the minimum listing details below before sending it to the admin team.</p><div className="review-readiness-list">{property.submissionIssues.map((issue) => <span key={issue}>• {issue}</span>)}</div></> : <p>The minimum review requirements are complete. Submission sends the current saved property record to the Find A Place admin queue.</p> : <p>{property.status === "PENDING_REVIEW" ? "The listing is locked while the admin team reviews the saved version." : property.status === "APPROVED" ? "Approval does not automatically make a listing public. An authorized admin must publish it separately." : property.status === "PUBLISHED" ? "The listing is guest-visible, but availability and checkout remain disabled until later verified milestones." : "This listing is not currently editable from the host side."}</p>}
