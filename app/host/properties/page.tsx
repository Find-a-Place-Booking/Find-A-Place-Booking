...
export default async function PropertiesPage({ searchParams }: { searchParams: Promise<{ error?: string }> }) {
  ...
  return (
    <DashboardShell active="Properties" title="Properties">
      ...
      <div className="feature-callout"><div><p className="eyebrow">Property workflow</p><h2>Property identity, review and publication are active; booking is not enabled yet.</h2><p>Each property keeps an immutable internal ID, stable shareable URL, structured details and private photo storage. Hosts submit for review, authorized admins control publication, and calendar availability/checkout remain disabled until those systems are ready.</p></div><div className="feature-checks"><span>✓ Stable booking slug</span><span>✓ Slug history</span><span>✓ Real photos</span><span>✓ Structured amenities</span><span>✓ Policies + rates</span><span>✓ Admin visibility</span></div></div>
    </DashboardShell>
  );
}
