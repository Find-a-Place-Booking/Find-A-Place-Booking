...
async function loadChunks<T>(
  ...
  for (const idsChunk of chunk(ids)) {
    ...
    if (result.error) {
      throw new Error(result.error.message || "Unable to load admin report records.");
    }
...
export async function getAdminReport(filters: Filters = {}) {
  ...
  if (organizationError || propertyError) {
    throw new Error("Unable to load report dimensions.");
  }

  const organizations = (organizationData ?? []) as {
    id: string;
    name: string;
    commission_tier: string;
  }[];
  const properties = (propertyData ?? []) as {
    id: string;
    organization_id: string;
    name: string;
    status: string;
  ...
  const organizationFilter =
    ...
    organizations.some((organization) => organization.id === filters.organizationId)
      ? filters.organizationId
      : null;
  ...
  let reservationQuery = supabase
    .from("reservations")
    .select(
      "id,organization_id,property_id,confirmation_code,guest_name,status,payment_status,check_in,check_out,tax_total_cents,platform_commission_cents,currency,created_at",
    )
  ...
  if (organizationFilter) {
    reservationQuery = reservationQuery.eq("organization_id", organizationFilter);
  }
  if (propertyFilter) {
  ...
  if (reservationError) {
    throw new Error("Unable to load admin reporting reservations.");
  }
  ...
  const payments = reservationIds.length
    ? await loadChunks<PaymentRow>(reservationIds, async (ids) => {
  ...
  const refunds = reservationIds.length
    ? await loadChunks<RefundRow>(reservationIds, async (ids) => {
        const result = await supabase
          .from("refunds")
  ...
  const notifications = reservationIds.length
    ? await loadChunks<{ id: string; reservation_id: string; status: string }>(
        reservationIds,
        async (ids) => {
          const result = await supabase
  ...
  for (const refund of refunds) {
    refundByReservation.set(
      refund.reservation_id,
      (refundByReservation.get(refund.reservation_id) ?? 0) +
  ...
  const rows = reservations.map((reservation) => {
    ...
    return {
      ...
      organizationName:
        organizationById.get(reservation.organization_id)?.name ||
        "Host organization",
      ...
      processingCents: settled
        ...
        : 0,
      processorActualCents: settled
        ? Number(payment.processor_fee_actual_cents || 0)
        : 0,
  ...
  function buildBreakdown(kind: "property" | "organization") {
    ...
    for (const row of rows) {
      ...
      if (row.status === "CONFIRMED") {
        ...
        bucket.bookedNights += stayNights(row.checkIn, row.checkOut);
      }
    ...
    return [...map.values()].sort(
      (a, b) => b.grossGuestCents - a.grossGuestCents,
    );
  }
  ...
  return {
    ...
    totals: {
      ...
      bookedNights: rows
        ...
        .reduce((sum, row) => sum + stayNights(row.checkIn, row.checkOut), 0),
