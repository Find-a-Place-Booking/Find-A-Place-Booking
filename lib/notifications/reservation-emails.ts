...
export async function sendBookingNotifications(
  ...
  const [propertyResult, organizationResult, policyAcceptanceResult] = await Promise.all([
    admin
      .from("properties")
    ...
    admin
      ...
      .select("platform_terms_version,cancellation_policy_version,property_policy_document_version,accepted_at")
  ...
  if (hostRecipient) {
    const emailVerified = reservation.guest_email_verified_at ? "Verified" : "Not verified";
    ...
    const agreementSummary = policyAcceptance?.accepted_at
      ? `${policyVersion}; Find A Place terms ${policyAcceptance.platform_terms_version}; cancellation policy ${policyAcceptance.cancellation_policy_version}`
    ...
    await sendOnce({
      ...
      text: `${guestName} booked ${propertyName} for ${reservation.check_in} through ${reservation.check_out}. Guest total: ${total}. Confirmation: ${reservation.confirmation_code}. Guest email: ${guestEmail} (${emailVerified}). Guest phone: ${guestPhone}. Identity: ${identityVerified}. Policies accepted: ${agreementSummary}. Open Find A Place: ${siteUrl()}/host/reservations/${reservation.id}`,
      html: `<p><strong>New Find A Place booking</strong></p><p>${escapeHtml(
        ...
        guestPhone,
      )}<br><strong>Identity:</strong> ${escapeHtml(identityVerified)}<br><strong>Policies:</strong> ${escapeHtml(agreementSummary)}</p><p>Find A Place only shares the verification result. Identity documents and selfies remain with Stripe Identity.</p><p><a href="${escapeHtml(
