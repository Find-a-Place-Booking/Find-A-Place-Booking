...
export async function sendDirectChargePaymentConfirmedNotification(
  ...
  const { data: reservation, error: reservationError } =
    await admin
      .from("reservations")
      .select(
        "id,confirmation_code,property_id,organization_id,guest_name,guest_total_cents,tax_total_cents,currency,platform_commission_cents,payment_environment",
      )
      .eq("id", reservationId)
      .single();
  ...
  if (reservationError || !reservation) {
    throw new Error(
      "Unable to load reservation payment notification context.",
    );
  }
  ...
  const [
    ...
    admin
      ...
      .eq(
        "payment_environment",
        reservation.payment_environment,
      )
      .single(),
    admin
      .from("properties")
  ...
  if (
    ...
    throw new Error(
      "Unable to load the confirmed direct-charge payment.",
    );
  }
  ...
  const currency =
    payment.currency || reservation.currency || "USD";
  ...
  await sendNotificationOnce({
    ...
    recipient,
    subject:
      `Payment confirmed: ${propertyName} · ${reservation.confirmation_code}`,
    text: [
      ...
      `Guest taxes in host charge: ${guestTax}`,
      "Find A Place retains no guest tax dollars from this payment.",
      `Stripe processing charged to host account: ${stripeProcessing}`,
      `Host net after Find A Place commission and Stripe processing: ${hostNet}`,
    ...
    )}</p><p><strong>Guest:</strong> ${escapeHtml(
      reservation.guest_name || "Guest",
    )}<br><strong>Guest paid:</strong> ${escapeHtml(
      guestPaid,
    )}<br><strong>Find A Place commission:</strong> ${escapeHtml(
      commission,
    )}<br><strong>Guest taxes in host charge:</strong> ${escapeHtml(
      guestTax,
    )}<br><strong>Stripe processing charged to host:</strong> ${escapeHtml(
      stripeProcessing,
    )}<br><strong>Host net:</strong> ${escapeHtml(
      hostNet,
    )}</p><p>Find A Place retains no guest tax dollars from this payment. Stripe owns the host balance and handles the host&apos;s normal bank-deposit timing.</p><p><a href="${escapeHtml(
      hostReservationUrl,
