...
export async function DashboardShell({
  ...
  return (
    <div
      className={`dashboard-layout ${portalStyles.portalShell}`}
      data-portal-shell="host"
    >
      ...
      <main className="dash-main">
        ...
        <header className="dash-topbar">
          <div>
            <small>{eyebrow || "Host dashboard"}</small>
            <h1>{title}</h1>
          </div>
          <div className="dash-actions">
            ...
            <Link
              ...
              href="/host/messages"
              aria-label="Reservation messages"
            >
              •
            </Link>
            <Link
              className={`avatar ${profile.avatarUrl ? "has-image" : ""}`}
              href="/host/settings"
