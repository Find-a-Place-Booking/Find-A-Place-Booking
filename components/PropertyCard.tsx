  ...
  return (
    <article className={`property-card ${wide ? "property-wide" : ""}`}>
      <Link className="property-image-wrap" href={`/stays/${property.slug}`}>
        {property.image ? <img className="property-image" src={property.image} alt={`${property.name} in ${property.location}`} /> : <div className="property-image property-image-empty">Photo coming soon</div>}
      ...
      <div className="property-body">
        <div className="property-kicker"><span>{property.location}</span><span>{property.reviews > 0 ? `★ ${property.rating}` : "New on Find A Place"}</span></div>
        <Link href={`/stays/${property.slug}`}><h3>{property.name}</h3></Link>
        <p>{property.tags.slice(0, 3).join(" · ")}</p>
        <div className="price-line"><strong>${property.price}</strong> / night <span>{property.reviews > 0 ? `${property.reviews} reviews` : "Independent stay"}</span></div>
      </div>
    </article>
  );
}
