...
const links: MobileLink[] = [
  ...
  ["Taxes & remittance", "/admin/taxes", "taxes", ["SUPER_ADMIN", "FINANCE_ADMIN"]],
  ["Site content", "/admin/content", "content", ["SUPER_ADMIN", "OPERATIONS_ADMIN"]],
...
export function AdminMobileNav({ active, roles }: { active: string; roles: AdminRole[] }) {
  ...
  return (
    <details className="internal-mobile-nav admin-mobile-nav">
      ...
      <nav>
        {visibleLinks.map(([label, href, key]) => <Link className={active === key ? "active" : ""} href={href} key={key}>{label}<span>›</span></Link>)}
